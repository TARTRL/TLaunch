...
### 1.Basic Example
[test_worker_comsumer.py](https://github.com/TARTRL/TLaunch/blob/main/examples/test_k8s/test_worker_comsumer.py) gives an example of worker and consumer , modified from the `consumer_producer` example on Deepmind launchpad's
...
### 2.Use GPU
We can customize the configuration information by adding a config object for node. For [example](https://github.com/TARTRL/TLaunch/blob/main/examples/test_k8s/test_gpu.py), when we want to apply gpu resources for node, we can refer to the following example.
